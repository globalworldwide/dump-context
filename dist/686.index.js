...
__webpack_require__.d(__webpack_exports__, {
  "__debug": () => (/* binding */ debugApis),
  "__internal": () => (/* binding */ sharedWithCli),
  "check": () => (/* binding */ check),
  "clearConfigCache": () => (/* binding */ clearCache3),
  "default": () => (/* binding */ src_default),
  "doc": () => (/* reexport */ doc_namespaceObject),
  "format": () => (/* binding */ format2),
  "formatWithCursor": () => (/* binding */ formatWithCursor2),
  "getFileInfo": () => (/* binding */ getFileInfo2),
  "getSupportInfo": () => (/* binding */ getSupportInfo2),
  "resolveConfig": () => (/* binding */ resolveConfig),
  "resolveConfigFile": () => (/* binding */ resolveConfigFile),
  "util": () => (/* binding */ prettier_public_exports),
  "version": () => (/* binding */ version_evaluate_default)
});

// NAMESPACE OBJECT: ./node_modules/prettier/doc.mjs
var doc_namespaceObject = {};
__webpack_require__.r(doc_namespaceObject);
__webpack_require__.d(doc_namespaceObject, {
  "builders": () => (builders),
...
var __defNormalProp = (obj, key2, value) => key2 in obj ? prettier_defProp(obj, key2, { enumerable: true, configurable: true, writable: true, value }) : obj[key2] = value;
var __require = /* @__PURE__ */ ((x) => typeof prettier_require !== "undefined" ? prettier_require : typeof Proxy !== "undefined" ? new Proxy(x, {
  ...
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key2 of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key2) && key2 !== except)
        prettier_defProp(to, key2, { get: () => from[key2], enumerable: !(desc = __getOwnPropDesc(from, key2)) || desc.enumerable });
  }
  return to;
};
...
function hasFlag(flag, argv = globalThis.Deno ? globalThis.Deno.args : external_process_.argv) {
  const prefix = flag.startsWith("-") ? "" : flag.length === 1 ? "-" : "--";
  const position = argv.indexOf(prefix + flag);
  const terminatorPosition = argv.indexOf("--");
  return position !== -1 && (terminatorPosition === -1 || position < terminatorPosition);
}
var { env } = external_process_;
var flagForceColor;
...
var supportsColor = {
  stdout: createSupportsColor({ isTTY: external_tty_.isatty(1) }),
  stderr: createSupportsColor({ isTTY: external_tty_.isatty(2) })
};
var supports_color_default = supportsColor;

// node_modules/chalk/source/utilities.js
function prettier_stringReplaceAll(string, substring, replacer) {
...
var applyStyle = (self, string) => {
  ...
  if (string.includes("\x1B")) {
    while (styler !== void 0) {
      string = prettier_stringReplaceAll(string, styler.close, styler.open);
...
var toPath = (urlOrPath) => isUrl(urlOrPath) ? (0,external_url_.fileURLToPath)(urlOrPath) : urlOrPath;
...
async function isDirectory(directory, options8) {
  ...
  let stats;
  try {
    stats = await (allowSymlinks ? promises_.stat : promises_.lstat)(toPath(directory));
  } catch {
    return false;
  }
...
var toAbsolutePath = (value) => external_path_.resolve(toPath(value));
function* iterateDirectoryUp(from, to) {
  from = toAbsolutePath(from);
  const { root: root2 } = external_path_.parse(from);
  to = to ? toAbsolutePath(to) : root2;
  if (from !== to && !from.startsWith(to)) {
    return;
  }
  for (let directory = from; directory !== to; directory = external_path_.dirname(directory)) {
    yield directory;
  }
  yield to;
}
...
searchInDirectory_fn = async function(directory, shouldCache) {
  ...
  for (const name of __privateGet(this, _names)) {
    const fileOrDirectory = external_path_.join(directory, name);
...
async function findProjectRoot(startDirectory, options8) {
  ...
  return mark ? external_path_.dirname(mark) : void 0;
}
...
async function loadEditorconfigInternal(file, { shouldCache }) {
  const directory = external_path_.dirname(file);
...
function loadEditorconfig(file, { shouldCache }) {
  file = external_path_.resolve(file);
...
function writeFormattedFile(file, data) {
  return promises_.writeFile(file, data);
}
var mockable = {
...
async function isFile(file, options8) {
  ...
  let stats;
  try {
    stats = await (allowSymlinks ? promises_.stat : promises_.lstat)(toPath(file));
  } catch {
    return false;
  }
  return stats.isFile();
...
var prettier_int = new type("tag:yaml.org,2002:int", {
  kind: "scalar",
...
var prettier_float = new type("tag:yaml.org,2002:float", {
  kind: "scalar",
...
function invalidChar(c2) {
  if (c2 === void 0) {
    return syntaxError(`JSON5: invalid end of input at ${prettier_line}:${column}`);
  }
  return syntaxError(`JSON5: invalid character '${formatChar(c2)}' at ${prettier_line}:${column}`);
}
function invalidEOF() {
  return syntaxError(`JSON5: invalid end of input at ${prettier_line}:${column}`);
}
function invalidIdentifier() {
  column -= 5;
  return syntaxError(`JSON5: invalid identifier character at ${prettier_line}:${column}`);
}
...
async function readFile(file) {
  ...
  try {
    return await promises_.readFile(file, "utf8");
  } catch (error) {
...
async function loadJs(file) {
  const module = await __webpack_require__(495)((0,external_url_.pathToFileURL)(file).href);
...
codes.ERR_INVALID_ARG_TYPE = createError(
  ...
  (name, expected, actual) => {
    external_assert_(typeof name === "string", "'name' must be a string");
...
codes.ERR_INVALID_PACKAGE_TARGET = createError(
  ...
  (pkgPath, key2, target, isImport = false, base = void 0) => {
    ...
    if (key2 === ".") {
      external_assert_(isImport === false);
...
function isErrorStackTraceLimitWritable() {
  try {
    if (external_v8_.startupSnapshot.isBuildingSnapshot()) {
      return false;
    }
  } catch {
  }
...
function getMessage(key2, args, self) {
  ...
  external_assert_(message !== void 0, "expected `message` to be found");
  ...
  return Reflect.apply(external_util_.format, null, args);
}
function determineSpecificType(value) {
  ...
  if (typeof value === "object") {
    ...
    return `${(0,external_util_.inspect)(value, { depth: -1 })}`;
  }
  let inspected = (0,external_util_.inspect)(value, { colors: false });
...
function read2(jsonPath, { base, specifier }) {
  ...
  let string;
  try {
    string = external_fs_.readFileSync(external_path_.toNamespacedPath(jsonPath), "utf8");
  } catch (error) {
    const exception2 = (
      /** @type {ErrnoException} */
      error
  ...
  if (string !== void 0) {
    ...
    try {
      ...
      const error = new ERR_INVALID_PACKAGE_CONFIG(
        jsonPath,
        (base ? `"${specifier}" from ` : "") + (0,external_url_.fileURLToPath)(base || specifier),
        cause.message
      );
      error.cause = cause;
      throw error;
    ...
    result.exists = true;
    if (prettier_hasOwnProperty.call(parsed, "name") && typeof parsed.name === "string") {
      result.name = parsed.name;
    }
    if (prettier_hasOwnProperty.call(parsed, "main") && typeof parsed.main === "string") {
      result.main = parsed.main;
    }
    if (prettier_hasOwnProperty.call(parsed, "exports")) {
      result.exports = parsed.exports;
    }
    if (prettier_hasOwnProperty.call(parsed, "imports")) {
      result.imports = parsed.imports;
    }
    if (prettier_hasOwnProperty.call(parsed, "type") && (parsed.type === "commonjs" || parsed.type === "module")) {
...
function getPackageScopeConfig(resolved) {
  let packageJSONUrl = new external_url_.URL("package.json", resolved);
  while (true) {
    ...
    const packageConfig = package_json_reader_default.read(
      (0,external_url_.fileURLToPath)(packageJSONUrl),
      { specifier: resolved }
    );
    ...
    packageJSONUrl = new external_url_.URL("../package.json", packageJSONUrl);
    if (packageJSONUrl.pathname === lastPackageJSONUrl.pathname) {
      break;
    }
  }
  const packageJSONPath = (0,external_url_.fileURLToPath)(packageJSONUrl);
  return {
...
function getFileProtocolModuleFormat(url2, _context, ignoreErrors) {
  ...
  const filepath = (0,external_url_.fileURLToPath)(url2);
...
function emitInvalidSegmentDeprecation(target, request, match, packageJsonUrl, internal, base, isTarget) {
  if (external_process_.noDeprecation) {
    return;
  }
  const pjsonPath = (0,external_url_.fileURLToPath)(packageJsonUrl);
  const double = doubleSlashRegEx.exec(isTarget ? target : request) !== null;
  external_process_.emitWarning(
    `Use of deprecated ${double ? "double slash" : "leading or trailing slash matching"} resolving "${target}" for module request "${request}" ${request === match ? "" : `matched to "${match}" `}in the "${internal ? "imports" : "exports"}" field module resolution of the package at ${pjsonPath}${base ? ` imported from ${(0,external_url_.fileURLToPath)(base)}` : ""}.`,
    "DeprecationWarning",
    "DEP0166"
  );
}
function emitLegacyIndexDeprecation(url2, packageJsonUrl, base, main) {
  ...
  if (format3 !== "module")
    return;
  const urlPath = (0,external_url_.fileURLToPath)(url2.href);
  const pkgPath = (0,external_url_.fileURLToPath)(new external_url_.URL(".", packageJsonUrl));
  const basePath = (0,external_url_.fileURLToPath)(base);
  if (!main) {
    external_process_.emitWarning(
...
Default "index" lookups for the main are deprecated for ES modules.`,
  ...
  } else if (external_path_.resolve(pkgPath, main) !== urlPath) {
    external_process_.emitWarning(
...
function tryStatSync(path13) {
  try {
    return (0,external_fs_.statSync)(path13);
  } catch {
    return new external_fs_.Stats();
  }
}
function fileExists(url2) {
  const stats = (0,external_fs_.statSync)(url2, { throwIfNoEntry: false });
...
function legacyMainResolve(packageJsonUrl, packageConfig, base) {
  ...
  if (packageConfig.main !== void 0) {
    guess = new external_url_.URL(packageConfig.main, packageJsonUrl);
    if (fileExists(guess))
      return guess;
    const tries2 = [
    ...
    while (++i2 < tries2.length) {
      guess = new external_url_.URL(tries2[i2], packageJsonUrl);
      if (fileExists(guess))
        break;
      guess = void 0;
    }
  ...
  while (++i < tries.length) {
    guess = new external_url_.URL(tries[i], packageJsonUrl);
    if (fileExists(guess))
      break;
    guess = void 0;
  }
  ...
  throw new ERR_MODULE_NOT_FOUND(
    (0,external_url_.fileURLToPath)(new external_url_.URL(".", packageJsonUrl)),
    (0,external_url_.fileURLToPath)(base)
  );
}
function finalizeResolution(resolved, base, preserveSymlinks) {
  ...
  let filePath;
  try {
    filePath = (0,external_url_.fileURLToPath)(resolved);
  } catch (error) {
    const cause = (
      /** @type {ErrnoException} */
      error
  ...
  if (stats.isDirectory()) {
    const error = new ERR_UNSUPPORTED_DIR_IMPORT(filePath, (0,external_url_.fileURLToPath)(base));
  ...
  if (!stats.isFile()) {
    const error = new ERR_MODULE_NOT_FOUND(
      ...
      base && (0,external_url_.fileURLToPath)(base),
      true
    );
    error.url = String(resolved);
    throw error;
  ...
  if (!preserveSymlinks) {
    const real = (0,external_fs_.realpathSync)(filePath);
    const { search, hash } = resolved;
    resolved = (0,external_url_.pathToFileURL)(real + (filePath.endsWith(external_path_.sep) ? "/" : ""));
...
function importNotDefined(specifier, packageJsonUrl, base) {
  return new ERR_PACKAGE_IMPORT_NOT_DEFINED(
    specifier,
    packageJsonUrl && (0,external_url_.fileURLToPath)(new external_url_.URL(".", packageJsonUrl)),
    (0,external_url_.fileURLToPath)(base)
  );
}
...
function throwInvalidSubpath(request, match, packageJsonUrl, internal, base) {
  const reason = `request is not a valid match in pattern "${match}" for the "${internal ? "imports" : "exports"}" resolution of ${(0,external_url_.fileURLToPath)(packageJsonUrl)}`;
  throw new ERR_INVALID_MODULE_SPECIFIER(
    request,
    reason,
    base && (0,external_url_.fileURLToPath)(base)
...
function resolvePackageTargetString(target, subpath, match, packageJsonUrl, base, pattern, internal, isPathMap, conditions) {
  ...
  const resolved = new external_url_.URL(target, packageJsonUrl);
  const resolvedPath = resolved.pathname;
  const packagePath = new external_url_.URL(".", packageJsonUrl).pathname;
  ...
  return new external_url_.URL(subpath, resolved);
}
...
function emitTrailingSlashPatternDeprecation(match, pjsonUrl, base) {
  ...
  const pjsonPath = (0,external_url_.fileURLToPath)(pjsonUrl);
  if (emittedPackageWarnings.has(pjsonPath + "|" + match))
    return;
  emittedPackageWarnings.add(pjsonPath + "|" + match);
  external_process_.emitWarning(
    `Use of deprecated trailing slash pattern mapping "${match}" in the "exports" field module resolution of the package at ${pjsonPath}${base ? ` imported from ${(0,external_url_.fileURLToPath)(base)}` : ""}. Mapping specifiers ending in "/" is no longer supported.`,
    "DeprecationWarning",
    "DEP0155"
  );
}
...
function packageImportsResolve(name, base, conditions) {
  if (name === "#" || name.startsWith("#/") || name.endsWith("/")) {
    ...
    throw new ERR_INVALID_MODULE_SPECIFIER(name, reason, (0,external_url_.fileURLToPath)(base));
  }
  let packageJsonUrl;
  const packageConfig = getPackageScopeConfig(base);
  if (packageConfig.exists) {
    packageJsonUrl = (0,external_url_.pathToFileURL)(packageConfig.pjsonPath);
...
function packageResolve(specifier, base, conditions) {
  if (external_module_.builtinModules.includes(specifier)) {
    return new external_url_.URL("node:" + specifier);
  }
  ...
  const packageConfig = getPackageScopeConfig(base);
  if (packageConfig.exists) {
    const packageJsonUrl2 = (0,external_url_.pathToFileURL)(packageConfig.pjsonPath);
  ...
  let packageJsonUrl = new external_url_.URL(
    "./node_modules/" + packageName + "/package.json",
    base
  );
  let packageJsonPath = (0,external_url_.fileURLToPath)(packageJsonUrl);
  let lastPath;
  do {
    const stat = tryStatSync(packageJsonPath.slice(0, -13));
    if (!stat.isDirectory()) {
      lastPath = packageJsonPath;
      packageJsonUrl = new external_url_.URL(
        (isScoped ? "../../../../node_modules/" : "../../../node_modules/") + packageName + "/package.json",
        packageJsonUrl
      );
      packageJsonPath = (0,external_url_.fileURLToPath)(packageJsonUrl);
      continue;
    }
    ...
    return new external_url_.URL(packageSubpath, packageJsonUrl);
  } while (packageJsonPath.length !== lastPath.length);
  throw new ERR_MODULE_NOT_FOUND(packageName, (0,external_url_.fileURLToPath)(base), false);
}
...
function moduleResolve(specifier, base, conditions, preserveSymlinks) {
  ...
  if (shouldBeTreatedAsRelativeOrAbsolutePath(specifier)) {
    resolved = new external_url_.URL(specifier, base);
  } else if (!isRemote && specifier[0] === "#") {
    resolved = packageImportsResolve(specifier, base, conditions);
  } else {
    try {
      resolved = new external_url_.URL(specifier);
    } catch {
      if (!isRemote) {
  ...
  external_assert_(resolved !== void 0, "expected to be defined");
...
function defaultResolve(specifier, context = {}) {
  ...
  external_assert_(parentURL !== void 0, "expected `parentURL` to be defined");
  throwIfInvalidParentURL(parentURL);
  let parsedParentURL;
  if (parentURL) {
    try {
      parsedParentURL = new external_url_.URL(parentURL);
    } catch {
    }
  }
  let parsed;
  try {
    parsed = shouldBeTreatedAsRelativeOrAbsolutePath(specifier) ? new external_url_.URL(specifier, parsedParentURL) : new external_url_.URL(specifier);
  ...
  const url2 = moduleResolve(specifier, new external_url_.URL(parentURL), conditions, false);
  return {
...
function importFromFile(specifier, parent) {
  ...
  return __webpack_require__(495)(url2);
}
...
function requireFromFile(id, parent) {
  const require2 = (0,external_module_.createRequire)(parent);
  return require2(id);
}
...
async function loadConfig(configFile) {
  const { base: fileName, ext: extension } = external_path_.parse(configFile);
...
function loadPrettierConfig(configFile, { shouldCache }) {
  configFile = external_path_.resolve(configFile);
...
function getSearchFunction(stopDirectory) {
  stopDirectory = stopDirectory ? external_path_.resolve(stopDirectory) : void 0;
...
function searchPrettierConfig(startDirectory, options8 = {}) {
  startDirectory = startDirectory ? external_path_.resolve(startDirectory) : process.cwd();
...
async function loadPrettierConfig2(file, options8) {
  ...
  if (!configFile) {
    const directory = file ? external_path_.dirname(external_path_.resolve(file)) : void 0;
...
async function resolveConfig(fileUrlOrPath, options8) {
  ...
  if (Array.isArray(merged.plugins)) {
    merged.plugins = merged.plugins.map(
      (value) => typeof value === "string" && value.startsWith(".") ? external_path_.resolve(external_path_.dirname(result.configFile), value) : value
    );
  }
  return merged;
}
async function resolveConfigFile(fileUrlOrPath) {
  const directory = fileUrlOrPath ? external_path_.dirname(external_path_.resolve(toPath(fileUrlOrPath))) : void 0;
...
function mergeOverrides(configResult, filePath) {
  ...
  if (filePath && overrides) {
    const relativeFilePath = external_path_.relative(external_path_.dirname(configFile), filePath);
...
var prettier_string_replace_all_default = stringReplaceAll2;
...
function getRelativePath(file, ignoreFile) {
  ...
  const filePath = isUrl(file) ? external_url_.fileURLToPath(file) : external_path_.resolve(file);
  return external_path_.relative(
    // If there's an ignore-path set, the filename must be relative to the
    // ignore path, not the current working directory.
    ignoreFilePath ? external_path_.dirname(ignoreFilePath) : process.cwd(),
    filePath
  );
}
...
function getInterpreter(file) {
  let fd;
  try {
    fd = external_fs_.openSync(file, "r");
  } catch {
    return;
  }
  try {
...
function prettier_getDocType(doc2) {
  ...
  if (prettier_VALID_OBJECT_DOC_TYPES.has(type2)) {
    return type2;
  }
}
...
Expected it to be 'string' or 'object'.`;
  ...
  const EXPECTED_TYPE_VALUES = prettier_disjunctionListFormat(
    [...prettier_VALID_OBJECT_DOC_TYPES].map((type3) => `'${type3}'`)
  );
...
var line2 = { type: prettier_DOC_TYPE_LINE };
...
// node_modules/get-east-asian-width/lookup.js
function prettier_isFullWidth(x) {
  return x === 12288 || x >= 65281 && x <= 65376 || x >= 65504 && x <= 65510;
}
function prettier_isWide(x) {
  return x >= 4352 && x <= 4447 || x === 8986 || x === 8987 || x === 9001 || x === 9002 || x >= 9193 && x <= 9196 || x === 9200 || x === 9203 || x === 9725 || x === 9726 || x === 9748 || x === 9749 || x >= 9800 && x <= 9811 || x === 9855 || x === 9875 || x === 9889 || x === 9898 || x === 9899 || x === 9917 || x === 9918 || x === 9924 || x === 9925 || x === 9934 || x === 9940 || x === 9962 || x === 9970 || x === 9971 || x === 9973 || x === 9978 || x === 9981 || x === 9989 || x === 9994 || x === 9995 || x === 10024 || x === 10060 || x === 10062 || x >= 10067 && x <= 10069 || x === 10071 || x >= 10133 && x <= 10135 || x === 10160 || x === 10175 || x === 11035 || x === 11036 || x === 11088 || x === 11093 || x >= 11904 && x <= 11929 || x >= 11931 && x <= 12019 || x >= 12032 && x <= 12245 || x >= 12272 && x <= 12287 || x >= 12289 && x <= 12350 || x >= 12353 && x <= 12438 || x >= 12441 && x <= 12543 || x >= 12549 && x <= 12591 || x >= 12593 && x <= 12686 || x >= 12688 && x <= 12771 || x >= 12783 && x <= 12830 || x >= 12832 && x <= 12871 || x >= 12880 && x <= 19903 || x >= 19968 && x <= 42124 || x >= 42128 && x <= 42182 || x >= 43360 && x <= 43388 || x >= 44032 && x <= 55203 || x >= 63744 && x <= 64255 || x >= 65040 && x <= 65049 || x >= 65072 && x <= 65106 || x >= 65108 && x <= 65126 || x >= 65128 && x <= 65131 || x >= 94176 && x <= 94180 || x === 94192 || x === 94193 || x >= 94208 && x <= 100343 || x >= 100352 && x <= 101589 || x >= 101632 && x <= 101640 || x >= 110576 && x <= 110579 || x >= 110581 && x <= 110587 || x === 110589 || x === 110590 || x >= 110592 && x <= 110882 || x === 110898 || x >= 110928 && x <= 110930 || x === 110933 || x >= 110948 && x <= 110951 || x >= 110960 && x <= 111355 || x === 126980 || x === 127183 || x === 127374 || x >= 127377 && x <= 127386 || x >= 127488 && x <= 127490 || x >= 127504 && x <= 127547 || x >= 127552 && x <= 127560 || x === 127568 || x === 127569 || x >= 127584 && x <= 127589 || x >= 127744 && x <= 127776 || x >= 127789 && x <= 127797 || x >= 127799 && x <= 127868 || x >= 127870 && x <= 127891 || x >= 127904 && x <= 127946 || x >= 127951 && x <= 127955 || x >= 127968 && x <= 127984 || x === 127988 || x >= 127992 && x <= 128062 || x === 128064 || x >= 128066 && x <= 128252 || x >= 128255 && x <= 128317 || x >= 128331 && x <= 128334 || x >= 128336 && x <= 128359 || x === 128378 || x === 128405 || x === 128406 || x === 128420 || x >= 128507 && x <= 128591 || x >= 128640 && x <= 128709 || x === 128716 || x >= 128720 && x <= 128722 || x >= 128725 && x <= 128727 || x >= 128732 && x <= 128735 || x === 128747 || x === 128748 || x >= 128756 && x <= 128764 || x >= 128992 && x <= 129003 || x === 129008 || x >= 129292 && x <= 129338 || x >= 129340 && x <= 129349 || x >= 129351 && x <= 129535 || x >= 129648 && x <= 129660 || x >= 129664 && x <= 129672 || x >= 129680 && x <= 129725 || x >= 129727 && x <= 129733 || x >= 129742 && x <= 129755 || x >= 129760 && x <= 129768 || x >= 129776 && x <= 129784 || x >= 131072 && x <= 196605 || x >= 196608 && x <= 262141;
}

// node_modules/get-east-asian-width/index.js
var prettier_isNarrowWidth = (codePoint) => !(prettier_isFullWidth(codePoint) || prettier_isWide(codePoint));
...
function prettier_getStringWidth(text) {
  ...
  for (const character of text) {
    ...
    width += prettier_isNarrowWidth(codePoint) ? 1 : 2;
  }
  return width;
}
  ...
  while (cmds.length > 0) {
    ...
    switch (prettier_get_doc_type_default(doc3)) {
      case prettier_DOC_TYPE_STRING: {
        ...
        out.push(formatted);
        if (cmds.length > 0) {
          pos2 += prettier_get_string_width_default(formatted);
        }
        break;
      }
      case prettier_DOC_TYPE_ARRAY:
...
function breakTies(tiesToBreak, options8) {
  ...
  for (indexOfFirstLeadingComment = tieCount; indexOfFirstLeadingComment > 0; --indexOfFirstLeadingComment) {
    ...
    external_assert_.strictEqual(currentCommentPrecedingNode, precedingNode);
    external_assert_.strictEqual(currentCommentFollowingNode, followingNode);
...
var { parsers, printers } = createParsersAndPrinters([
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 362).then(__webpack_require__.bind(__webpack_require__, 9362)),
    parsers: ["acorn", "espree"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 653).then(__webpack_require__.bind(__webpack_require__, 5129)),
    parsers: [
      "__ng_action",
      "__ng_binding",
      "__ng_interpolation",
  ...
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 100).then(__webpack_require__.bind(__webpack_require__, 100)),
    parsers: [
      "babel",
      "babel-flow",
      "babel-ts",
  ...
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 437).then(__webpack_require__.bind(__webpack_require__, 1437)),
    printers: ["estree", "estree-json"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 156).then(__webpack_require__.bind(__webpack_require__, 4156)),
    parsers: ["flow"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 129).then(__webpack_require__.bind(__webpack_require__, 2129)),
    parsers: ["glimmer"],
    printers: ["glimmer"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 692).then(__webpack_require__.bind(__webpack_require__, 4692)),
    parsers: ["graphql"],
    printers: ["graphql"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 194).then(__webpack_require__.bind(__webpack_require__, 2194)),
    parsers: ["html", "angular", "vue", "lwc"],
    printers: ["html"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 935).then(__webpack_require__.bind(__webpack_require__, 2935)),
    parsers: ["markdown", "mdx", "remark"],
    printers: ["mdast"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 331).then(__webpack_require__.bind(__webpack_require__, 7331)),
    parsers: ["meriyah"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 870).then(__webpack_require__.bind(__webpack_require__, 2870)),
    parsers: ["css", "less", "scss"],
    printers: ["postcss"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 679).then(__webpack_require__.bind(__webpack_require__, 1679)),
    parsers: ["typescript"]
  },
  {
    importPlugin: () => __webpack_require__.e(/* import() */ 511).then(__webpack_require__.bind(__webpack_require__, 7511)),
    parsers: ["yaml"],
    printers: ["yaml"]
  }
]);
...
function importFromDirectory(specifier, directory) {
  return import_from_file_default(specifier, external_path_.join(directory, "noop.js"));
}
...
async function importPlugin(name, cwd) {
  if (external_path_.isAbsolute(name)) {
    return __webpack_require__(495)((0,external_url_.pathToFileURL)(name).href);
  }
  try {
    return await __webpack_require__(495)((0,external_url_.pathToFileURL)(external_path_.resolve(name)).href);
  } catch {
